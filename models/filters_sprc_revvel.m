function data = filters_sprc_revvel(processed, nK_sp, nK_vel, dt_sp, dt_vel)
	...
	%
	%Usage:
	%	data = filters_sprc_revvel(processed, nK_sp, nK_vel, dt_sp, dt_vel)
	%     
	%Input:
	...
	%	dt_sp = 0.002;
	%	dt_vel = 0.05;
	%	data = filters_sprc_revvel(pre.processed, nK_sp, nK_vel, dt_sp, dt_vel);
	...
	for idx=1:nU 
		...
		for j = (strpt):(nB)
			...
			torqueRU = processed.dtorque(j-(nK_vel-1)*steps_vel:steps_vel:(j),1);
			torqueFE = processed.dtorque(j-(nK_vel-1)*steps_vel:steps_vel:(j),2);
