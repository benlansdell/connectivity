function data = filters_sprc_revpos_lv(processed, nK_sp, nK_pos, dt_sp, dt_pos)
	%Prepare spike and torque data for GLM which includes spike history and cursor position (x_1, x_2) filters:
	%Position filter is retroactive, in that it encodes past trajectory values, not future
	...
	%
	%Usage:
	%	data = filters_sprc_revpos_lv(processed, nK_sp, nK_pos, dt_sp, dt_pos)
	%     
	%Input:
	...
	%	dt_sp = 0.002;
	%	dt_pos = 0.05;
	%	data = filters_sprc_revpos_lv(pre.processed, nK_sp, nK_pos, dt_sp, dt_pos);
	...
	for idx=1:nU 
		...
		for j = (strpt):(nB)
			...
			%(past) torque trajectory
			torqueRU = processed.cursor(j-(nK_pos-1)*steps_pos:steps_pos:(j),1);
			torqueFE = processed.cursor(j-(nK_pos-1)*steps_pos:steps_pos:(j),2);
