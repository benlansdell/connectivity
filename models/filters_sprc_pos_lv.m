function data = filters_sprc_pos_lv(processed, nK_sp, nK_pos, dt_sp, dt_pos)
	%Prepare spike and cursor data for GLM which includes spike history and cursor position (x_1, x_2) filters:
	%Uses labview cursor data, instead of torque data.
	...
	%
	%Usage:
	%	data = filters_sprc_pos_lv(processed, nK_sp, nK_pos, dt_sp, dt_pos)
	%     
	%Input:
	...
	%	dt_sp = 0.002;
	%	dt_pos = 0.05;
	%	data = filters_sprc_pos_lv(pre.processed, nK_sp, nK_pos, dt_sp, dt_pos);
