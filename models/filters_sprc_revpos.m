function data = filters_sprc_revpos(processed, nK_sp, nK_pos, dt_sp, dt_pos)
	...
	%
	%Usage:
	%	data = filters_sprc_revpos(processed, nK_sp, nK_pos, dt_sp, dt_pos)
	%     
	%Input:
	...
	%	dt_sp = 0.002;
	%	dt_pos = 0.05;
	%	data = filters_sprc_revpos(pre.processed, nK_sp, nK_pos, dt_sp, dt_pos);
	...
	for idx=1:nU 
		...
		for j = (strpt):(nB)
			...
			torqueRU = processed.torque(j-(nK_pos-1)*steps_pos:steps_pos:(j),1);
			torqueFE = processed.torque(j-(nK_pos-1)*steps_pos:steps_pos:(j),2);
