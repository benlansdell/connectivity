function data = filters_sprc_vel(processed, nK_sp, nK_vel, dt_sp, dt_vel)
	...
	%
	%Usage:
	%	data = filters_sprc_vel(processed, nK_sp, nK_vel, dt_sp, dt_vel)
	%     
	%Input:
	...
	%	dt_sp = 0.002;
	%	dt_vel = 0.05;
	%	data = filters_sprc_vel(pre.processed, nK_sp, nK_vel, dt_sp, dt_vel);
	...
	for idx=1:nU 
		...
		for j = (nK_sp*steps_sp+1):(nB-nK_vel*steps_vel)
			...
			torqueRU = processed.dtorque(j:steps_vel:(j+nK_vel*steps_vel-1),1);
			torqueFE = processed.dtorque(j:steps_vel:(j+nK_vel*steps_vel-1),2);
