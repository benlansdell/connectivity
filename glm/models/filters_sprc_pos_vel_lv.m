function data = filters_sprc_pos_vel_lv(processed, nK_sp, nK_pos, nK_vel, dt_sp, dt_pos, dt_vel)
	...
	%
	%Usage:
	%	data = filters_sprc_pos_vel_lv(processed, nK_sp, nK_pos, dt_sp, dt_pos)
	%     
	%Input:
	...
	%	dt_pos = 0.05;
	%	dt_vel = 0.05;
	%	data = filters_sprc_pos_vel_lv(pre.processed, nK_sp, nK_pos, nK_vel, dt_sp, dt_pos, dt_vel);
	...
	data.k{3,3} = dt_pos;
	data.k{4,1} = 'RU vel'; 
	data.k{4,2} = (1:nK_vel) + nK_rc + 2*nK_pos;
	data.k{4,3} = dt_vel;
	data.k{5,1} = 'FE vel'; 
	data.k{5,2} = (1:nK_vel) + nK_rc + 2*nK_pos + nK_vel;
	data.k{5,3} = dt_vel;
	...
	for idx=1:nU 
		...
		for j = startbin:endbin
			...
			dtorqueRU = processed.dcursor(j:steps_vel:(j+(nK_vel-1)*steps_vel),1);
			dtorqueFE = processed.dcursor(j:steps_vel:(j+(nK_vel-1)*steps_vel),2);
			%Form stim vector
