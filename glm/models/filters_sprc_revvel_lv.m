function data = filters_sprc_revvel_lv(processed, nK_sp, nK_vel, dt_sp, dt_vel)
	%Prepare spike and torque data for GLM which includes spike history and cursor position (x_1, x_2) filters:
	%Velocity filter is retroactive, in that it encodes past trajectory values, not future
	...
	%
	%Usage:
	%	data = filters_sprc_revvel_lv(processed, nK_sp, nK_vel, dt_sp, dt_vel)
	%     
	%Input:
	...
	%	dt_sp = 0.002;
	%	dt_vel = 0.05;
	%	data = filters_sprc_revvel_lv(pre.processed, nK_sp, nK_vel, dt_sp, dt_vel);
	...
	strpt = 1+max(nK_sp*steps_sp,nK_vel*steps_vel);
	...
	for idx=1:nU 
		...
		for j = (strpt):(nB)
			...
			torqueRU = processed.dcursor(j-(nK_vel-1)*steps_vel:steps_vel:(j),1);
			torqueFE = processed.dcursor(j-(nK_vel-1)*steps_vel:steps_vel:(j),2);
	...
	data.X = data.X(:,strpt:(nB),:); %(nkt+1:end-nkt,:);
	data.y = processed.binnedspikes(strpt:(nB), :)';
