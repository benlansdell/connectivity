function data = filters_sprc_forrevpos(processed, nK_sp, nK_pos, dt_sp, dt_pos)
	%Prepare spike and torque data for GLM which includes spike history and cursor position (x_1, x_2) filters:
	%Position filter is both antero and retro-active, in that it encodes past trajectory values, not future
	...
	%
	%Usage:
	%	data = filters_sprc_forrevpos(processed, nK_sp, nK_pos, dt_sp, dt_pos)
	%     
	%Input:
	...
	%	dt_sp = 0.002;
	%	dt_pos = 0.05;
	%	data = filters_sprc_forrevpos(pre.processed, nK_sp, nK_pos, dt_sp, dt_pos);
